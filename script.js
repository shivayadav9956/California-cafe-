function findCaliforniaCafes(searchTerm) {
  ...
  function searchCafes() {
    ...
    if (results.length === 0) {
      resultsContainer.innerHTML = '<p>No matching cafes found.</p>';
      return;
    }
    ...
    results.forEach(result => {
      const cafeDetails = `
        <div>
          ...
          <p><strong>Street No:</strong> ${result.street_no}</p>
          <p><strong>Locality:</strong> ${result.locality}</p>
          <p><strong>Postal Code:</strong> ${result.postal_code}</p>
          <p><strong>Latitude:</strong> ${result.lat}</p>
          <p><strong>Longitude:</strong> ${result.long}</p>
        </div>
      `;
      resultsContainer.innerHTML += cafeDetails;
    });
  }